...
## Data Processing
All preprocessing methods are implemented in **`dataset.py`**.  
The dataset is sourced from the **HipMRI Study** for prostate cancer radiotherapy and contains 2D grayscale MRI slices of the pelvis, stored in **`keras_slices_data/`**.

Images are primarily **256×128 pixels**; any slices with inconsistent dimensions were removed.  
After preprocessing, the dataset was split as follows:
- **Training:** 11,460 images  
- **Validation:** 660 images  
- **Test:** 540 images  

## Data Pipeline
2D MRI slices were loaded from NIfTI files using **Nibabel**, then preprocessed with **NumPy** for intensity normalization and size filtering. Each image was **z-score normalized**, **reshaped to 256×128**, and converted to **PyTorch tensors** with an added channel dimension. **PyTorch DataLoaders** were used for batching, shuffling, and efficient data loading during training, validation, and testing.
...
### Loss Function
The VQ-VAE2 model optimizes a composite loss comprising three key terms:

![VQ-VAE2 Loss Function](images/loss_function.png)
*Figure 2: VQ-VAE-2 loss function.*

- **Reconstruction Loss** — Ensures decoded outputs match inputs (e.g., MSE or perceptual loss).  
- **Codebook Loss** — Updates embeddings toward encoder outputs to improve representation.  
- **Commitment Loss** — Keeps encoder outputs close to chosen codebook vectors, scaled by **β** (≈0.25–0.5). 
...
![Blocks Used (Encoder, Decoder and VQ)](images/block_diagram.png)
*Figure 3: VQ-VAE-2 blocks used.*
...
## Training
The VQ-VAE-2 model is trained for *80 epochs* using the Adam optimizer, optimizing a combination of reconstruction (MSE) and vector quantization losses. Validation loss and SSIM are monitored at each epoch to track reconstruction quality, and the final trained model, along with loss and SSIM plots, are saved in the train_dir folder.

A *learning rate* of 1e-4 was selected after testing 1e-3 and 1e-5, as it provided the best balance between convergence speed and stability. The *commitment cost* was set to 0.25 based on literature recommendations and empirical tuning.

The model uses a *latent dimension* of 256, chosen to balance reconstruction fidelity with model complexity—large enough to capture structural details from the MRI slices while remaining efficient in memory and computation.
...
*Figure 5: Validation SSIM vs Epoch*
